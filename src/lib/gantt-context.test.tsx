...
function wrapper({ children }: { children: ReactNode }) {
	return <GanttProvider initialProject={makeProject()}>{children}</GanttProvider>;
}
...
describe("GanttProvider dirty tracking", () => {
	it("stays clean until a mutation occurs", () => {
		const { result } = renderHook(() => useGantt(), { wrapper });
		expect(result.current.isDirty).toBe(false);
	});
	...
	it("marks dirty after addWorkstream", () => {
		...
		expect(
			result.current.project.workstreams.some((w) => w.label === "Design"),
		).toBe(true);
	});
	...
	it("markClean resets isDirty back to false", () => {
		const { result } = renderHook(() => useGantt(), { wrapper });
...
describe("GanttProvider workstream operations", () => {
	it("addWorkstream adds a new workstream", () => {
		const { result } = renderHook(() => useGantt(), { wrapper });
	...
	it("updateWorkstream updates an existing workstream's fields", () => {
		const { result } = renderHook(() => useGantt(), { wrapper });
...
describe("GanttProvider work item operations", () => {
	...
	it("moveWorkItemToWorkstream moves an item between workstreams", () => {
		...
		const qaWs = result.current.project.workstreams.find(
			(w) => w.label === "QA",
		);
		expect(qaWs).toBeDefined();

		act(() => {
			result.current.moveWorkItemToWorkstream("item-1", qaWs!.id);
		});
...
describe("GanttProvider dependency operations", () => {
	it("addDependency creates a dependency between two items", () => {
		...
		act(() => {
			result.current.addWorkItem("ws-1", "Second Task", "2026-02-01", "2026-02-05");
		});
		...
		expect(dep.fromItemId).toBe("item-1");
		expect(dep.toItemId).toBe(secondItem!.id);
	});
...
describe("GanttProvider legend operations", () => {
	it("addLegendEntry adds a new legend entry", () => {
		...
		expect(
			result.current.project.legend.some((l) => l.label === "Milestone"),
		).toBe(true);
	});
...
describe("GanttProvider UI state setters", () => {
	...
	it("updates connectingFrom", () => {
		const { result } = renderHook(() => useGantt(), { wrapper });
		act(() => {
			result.current.setConnectingFrom("item-1");
		});
